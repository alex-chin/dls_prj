...
- получение дообученной модели yolov5 на закрытом датасете;
- выбор датасета похожего на закрытый - вид сверху на людей на открытой местности;
- конвертация исходной модели для портирования на Edge TPU;
- сравнение производительности работы исходной модели на CPU и TPU на похожем датасете;

Не является целью проекта - увеличение качества метрик, но расчет метрик предполагается.

## 1. Установка и настройка Coral Edge TPU

Архитектура программной поддержки аппаратных средств

* **Edge TPU Runtime (`libedgetpu.dll`)**:
    * Основной программный интерфейс для взаимодействия с аппаратным ускорителем Edge TPU.
    * Обязательный компонент для использования любого устройства Edge TPU на Windows.

* **PyCoral Library**:
    * Библиотека Python, разработанная Google для упрощения работы с Edge TPU.
    * Предоставляет удобные высокоуровневые API для выполнения инференса TensorFlow Lite моделей на Edge TPU.
    * Упрощает такие задачи, как загрузка моделей, выполнение выводов и постобработка результатов.

* **TensorFlow Lite**:
    * Edge TPU предназначен для работы с моделями в формате TensorFlow Lite.
    * Модели должны быть предварительно **скомпилированы специально для Edge TPU** с использованием компилятора Edge
      TPU, чтобы максимально использовать аппаратные возможности ускорителя.

Для установки программных средств необходимо выполнить последовательно пункты из [init.ipynb](init.ipynb)

1. Установка драйверов
2. Установка API PyCoral
3. Запуск контрольных примеров

## 2. Настройка конвертации

Настройка конвертации ([yolo_export.ipynb](yolo_export.ipynb)) проводилась на
сервере [Colab](https://colab.research.google.com/),
использовался программный пакет YOLO из системы ultralytics

Установка пакета проводилась в два этапа:

1. `pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu`
2. `pip install ultralytics`

Это позволило ускорить установку и уменьшить объем скачиваемых пакетов на 2.5 Гб. В этом случае не скачиваются пакеты
поддержки видео ускорителей так как для конвертации они ненужны.

Время конвертации от 15 до 30 мин.

## 3. Конвертация модели обученной на датасете детектирования людей на открытой местности

Путем экспериментов удалось обнаружить что успешно конвертируются и выполняется инференс на устройстве стандартные
модели YOLOv5, YOLOv6, YOLOv7, YOLOv7 с разрешением 640x640.

**Финальная модель best_9.pt (YOLOv5) с разрешением 640x640 прошла полную конвертацию за 5 мин.**

## 4. Анализ эффективности выполнение вывода модели на TPU

В ноутбуке [validate_yolov5.ipynb](validate_yolov5.ipynb) проводится проверка и валидация работы модели YOLOv5 
для задачи детекции объектов. В
начале ноутбука осуществляется загрузка необходимых библиотек, таких как ultralytics (YOLO), а также подготовка данных и
модели.

Был выбран следующий датасет [heridal-yfx5o](https://universe.roboflow.com/aautonomy/heridal-yfx5o/dataset/1). Он состоит 
из 1579 размеченных изображений 640x640 общим объемом 170MB. 
Далее в ноутбуке реализуется процесс инференса: изображения из тестового набора пропускаются через модель, и результаты
сохраняются для последующего анализа. Для удобства визуализации используются инструменты matplotlib и OpenCV,
позволяющие отобразить изображения с наложенными предсказанными bounding box-ами и метками классов. Также выводится
информация о количестве обнаруженных объектов, их классах, координатах и уровне уверенности модели.

В заключительной части ноутбука проводится оценка качества работы модели с помощью стандартных метрик, таких как
точность (precision), полнота (recall) и mAP (mean Average Precision). Эти показатели позволяют объективно сравнить
различные версии моделей и выбрать оптимальную для дальнейшего использования. 

|     | Precision | Recall | F1     | mAP@0.5 | Inference mean ms |
|-----|-----------|--------|--------|---------|-------------------|
| CPU | 0.559     | 0.357  | 0.436  | 0.262   | 82                |
| TPU | 0.292     | 0.221  | 0.251  | 0.106   | 67                |


Модель показывает довольно низкие метрики. Это вызвано, тем что проверка вывода происходила на новых данных не связанных 
с обучением. Вывод на CPU осуществлялся до конвертации. 
Скорость вывода выше на TPU даже если сравнивать c компьютером со следующей конфигурацией: Core I-7, 16Gb RAM  